...
import { UserCog, Mail, Lock, Eye, EyeOff, ShieldCheck } from "lucide-react";
...
export default function AdminLoginPage() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    setError("");
    login('admin', { email, name: 'Admin User' });
  };
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-6 selection:bg-primary selection:text-primary-foreground">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_100%,rgba(25,133,161,0.05),transparent_50%)] pointer-events-none" />
      ...
      <Card className="w-full max-w-md border-border/50 shadow-2xl shadow-black/10" variant="glass" hoverable={false}>
        <CardHeader className="text-center space-y-4">
          ...
          <CardTitle className="text-2xl italic tracking-tighter">Admin Portal</CardTitle>
          <CardDescription>Enter your credentials to manage company infrastructure</CardDescription>
        </CardHeader>
          ...
          <form className="space-y-4" onSubmit={handleLogin}>
            <div className="space-y-2">
              <div className="relative group">
                ...
                <Input
                  type="email"
                  placeholder="admin@noteflow.com"
                  className="pl-10"
                  value={email}
            ...
            <div className="pt-6 mt-6 border-t border-border/50 flex items-center gap-3">
              ...
              <p className="text-[10px] text-muted-foreground/60 font-medium leading-relaxed">
                <strong>Secured Session:</strong> You may use any combination for testing. All actions are logged for audit.
              </p>
            </div>
          </form>
        </CardContent>
